...
(*Helper that gets the attacking and defending territories from the user
  and outputs them as a tuple*)
...
(* Helper that outputs the game state after moving the current player to the
    end of the player list*)
let turn_change 
(state : t) : t =
  let hd_to_tl =
...
let get_player_from_territory 
(g : t) 
(ter : territory) : player =
...
let rec owns_continent 
(local : territory list) 
(continent : territory list) : bool =
...
(m : int) 
(attacked_terr : territory) : int ref = 
  print_white
    "\n\
...
let rec num_abstraction 
(s : t) 
(attacked_terr : territory) 
(t : territory) : t * territory list = 
  print_white
    "\n\
...
(s : t) 
(player : player) 
(string : string) : t * territory list = 
  print_white
