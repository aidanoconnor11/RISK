...
let capture state t1 t2 =
  ...
  let rec player_list = function
    | [] -> []
    | h :: t -> if h = p2 then p2_update :: t else h :: player_list t
  in
  ...
  let p1_update =
    {
      p1 with
      territories =
        t2 :: update_list (get_territories (List.hd g1.players)) t1 (-x);
    }
  in
  ...
  if List.length p2.territories > 1 then { g2 with players = final_lst }
...
let battle_decision state d1 d2 t1 t2 =
  ...
  let rec compare_dice g rolls =
    ...
    else if List.hd first > List.hd second then
      compare_dice (updated_armies g t1 t2) (List.tl first, List.tl second)
    else compare_dice (updated_armies g t2 t1) (List.tl first, List.tl second)
  in
  compare_dice state rolls

let rec get_num_dice () =
  ANSITerminal.print_string [ ANSITerminal.green ]
    "How many dice would you like to roll? If you are the attacking territory,\n\
    \   you may roll between one and three dice. If you are the defending \
     territory,\n\
...
let rec get_territory_fortify () g : territory * territory =
  ...
  try
    ...
    then (
      ...
      if
        List.exists
          (fun x -> Game__Board.get_territory_name x = t2)
          (get_territories (List.hd g.players))
        = false
      then (
        ANSITerminal.print_string [ ANSITerminal.green ]
